...
// Directly get maxRounds from query params during init, ensure child component gets value immediately
...
const handleGoBack = async () => {
  // Before returning to Step 2, close running simulation
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment not running, check if need to stop process
...
const handleNextStep = () => {
  // Step3Simulation component directly handles report generation and route navigation
  ...
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        addLog(`Failed to get time config, using default value: ${minutesPerRound.value} minutes/round`)
      }
...
onMounted(() => {
  ...
  // Record maxRounds config (value already obtained from query params during init)
  if (maxRounds.value) {
