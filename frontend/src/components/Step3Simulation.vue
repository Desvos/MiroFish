...
// Computed
// Display actions in chronological order (newest at the end, i.e., bottom)
...
const doStartSimulation = async () => {
  ...
  // First reset all states to ensure no influence from previous simulation
  resetAllState()
...
// Track previous round of each platform for detecting changes and outputting logs
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform is completed, check if all enabled platforms are completed
  // Determine if platform is enabled by actions_count (if count > 0 or running was true)
