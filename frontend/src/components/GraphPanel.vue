    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG long-term memory updating in real-time' : 'Updating in real-time...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing, recommended to manually refresh graph later</span>
...
// Listen to isSimulating changes, detect simulation end
...
const renderGraph = () => {
  ...
  // Record which edge of each node pair being processed
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distributed curvature for clear distinction
      // Curvature range increases with edge count
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if edge direction opposite to normalized direction
      // This ensures all edges distribute in same reference frame, avoiding overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to gather isolated node groups to center area
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset to connection direction, calculated based on distance ratio to ensure visible curve
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Check if real drag started (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Only restart simulation on first real drag detection
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only let simulation gradually stop after real drag
        if (d._isDragging) {
  ...
  // Click empty area to close details panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// Listen to edge label visibility toggle
