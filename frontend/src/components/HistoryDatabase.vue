<template>
  ...
  >
    <!-- Background decoration: tech grid lines (only show when has projects) -->
    ...
    <!-- Card container (only show when has projects) -->
...
// Card layout config - adjusted to wider ratio
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards in current row, ensure centering
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (record latest state regardless of animation)
        ...
        // Use debounce to delay state change, prevent rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs execution (may have been overridden)
          ...
          setTimeout(() => {
            ...
            // After animation, check for new pending states
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom, need more scroll to trigger expand
...
.grid-pattern {
  ...
  /* Position from top-left, expand at bottom when height changes without affecting existing grid */
