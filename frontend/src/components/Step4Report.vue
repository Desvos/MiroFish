            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, but entire section may not be complete) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When it's the final answer, show special hint -->
...
const toolConfig = {
  ...
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Zap icon - represents quick search
  },
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interview subject selection reason
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: - Select Parent_601 (index 0): As parent group representative...
        if (!headerMatch) {
        ...
        } else if (currentName && line.trim() && !line.match(/^Not selected|^In summary|^最终Select/)) {
          // Reason continuation line (exclude ending summary paragraph)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If first question has "1.", needs special handling
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform markers (very old format), use whole as answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes (compatible with multiple quote formats)
      const quotesMatch = block.match(/\*\*Key quote:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Priority match > "text" format
    ...
    const summaryMatch = text.match(/### InterviewSummary与核心观点\n([\s\S]*?)$/)
    if (summaryMatch) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Current Key Facts (${props.result.facts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'insight-content' }, [
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No current key facts'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Currently Valid Memory (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Currently Valid Memory'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check if it's platform placeholder text
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" or "Question X:" (Chinese format, backend new format)
      // 2. "1. " or "\n1. " (number+dot, old format compatible)
      let matches = []
      let match

      // Priority try "Question X:" format
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // If no numbers found or only one found, return whole
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch button (only show when real dual-platform answers exist)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 (## xxx) since section title is already displayed outside
  ...
  // Clean <br> tags before/after block elements
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Clean <p><br> followed by block elements (caused by extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> before paragraph start tags after block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: when single-item <ol> is separated by paragraph content, keep numbering incremental
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Final Answer: (Chinese)
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Dark gray, doesn't change with state */
  font-weight: 500;
}
