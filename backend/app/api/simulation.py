...
# Adding this prefix prevents Agent from calling tools and responds directly with text
INTERVIEW_PROMPT_PREFIX = "Based on your persona, all past memories and actions, respond to me directly with text without calling any tools:"
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes that match predefined entity types (nodes with Labels other than just Entity)
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    """Get detailed info of single entity"""
    try:
        if not Config.ZEP_API_KEY:
            return jsonify({
                "success": False,
...
def create_simulation():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project has not built graph, please call /api/graph/build"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # Detailed log
        logger.debug(f"Check simulation readiness: {simulation_id}, status={status}, config_generated={config_generated}")
        
        # If config_generated=True and files exist, consider ready
        # The following statuses indicate preparation is complete:
        # - ready: ready to run
        # - preparing: if config_generated=True indicates complete
        # - running: running, preparation already complete
        # - completed: completed, preparation already complete
        # - stopped: stopped, preparation already complete
...
def prepare_simulation():
    ...
    Features:
    - Auto detect completed preparation work to avoid regeneration
    - If already prepared, return existing results directly
    - Support force regeneration (force_regenerate=true)
    
    Steps:
    1. Check if there is completed preparation work
...
def get_prepare_status():
    ...
    try:
        ...
        # If simulation_id provided, check if preparation is already completed
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        if not task:
            # Task doesn't exist, but if simulation_id provided, check if preparation is completed
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_history():
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get runtime status (read user-set actual rounds from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use user-set total_rounds, if not set use recommended rounds
            ...
            # Get related project files list (max 3)
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        # Check if currently generating (determine via state.json)
        is_generating = False
        total_expected = None
...
def start_simulation():
    ...
    try:
        ...
        # Smart status handling: allow restart if preparation is completed
        if state.status != SimulationStatus.READY:
            # Check if preparation is completed
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation completed, check if there are running processes
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows latest round for both platforms
...
def get_interview_history():
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # If not specified, return both platforms' history
        agent_id = data.get('agent_id')
