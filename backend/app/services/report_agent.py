...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Log section content generation complete (only records content, not full section completion)"""
        self.log(
...
2. Retrieve information from the simulation graph in multiple dimensions
...
[Wide-Angle Search - Get Full View]
...
Call the OASIS simulation environment interview API to conduct real interviews with running simulation agents!
This is not LLM simulation, but calling real interview interfaces to get original responses from simulation agents.
...
Function flow:
1. Automatically read character files to understand all simulation agents
2. Intelligently select agents most relevant to the interview topic (students, media, officials, etc.)
...
[Use Cases]
- Need to understand event perspectives from different character views (what do students think? What does media say? What do officials say?)
...
[Core Concept]
We have built a simulation world and injected specific "simulation requirements" as variables into it. The evolution of the simulation world is a prediction of what might happen in the future. You are not observing "experimental data" but a "rehearsal of the future".

[Your Task]
Write a "Future Prediction Report" that answers:
1. What happens in the future under our set conditions?
2. How do various Agents (population groups) react and act?
3. What future trends and risks does this simulation reveal that are worth paying attention to?

[Report Positioning]
- ✅ This is a simulation-based future prediction report, revealing "if this happens, what will happen in the future"
- ✅ Focus on prediction results: event trends, group reactions, emergence phenomena, potential risks
- ✅ Agent words and actions in the simulation world are predictions of future population behaviors
- ❌ Not an analysis of current real-world situations
- ❌ Not a general public opinion overview

[Section Count Limits]
- Minimum 2 sections, maximum 5 sections
- No subsections needed, each section should be complete content
...
[Some Future Facts Predicted by Simulation]
{related_facts_json}

Please examine this future rehearsal from the "God's eye view":
1. What state does the future present under our set conditions?
2. How do various population groups (Agents) react and act?
3. What future trends does this simulation reveal that are worth paying attention to?

Design the most appropriate report section structure based on prediction results.

[Reminder Again] Report section count: minimum 2, maximum 5, content should be concise and focused on core prediction findings."""

# ── Section Generation Prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
You are an expert in writing "Future Prediction Reports", writing one section of the report.
...
The behaviors and interactions of Agents in the simulation are predictions of future population behaviors.
...
Do not write as an analysis of current real-world situations
...
1. [Must Call Tools to Observe Simulation World]
   - You are observing the rehearsal of the future from the "God's eye view"
   - All content must come from events and Agent words/actions that occurred in the simulation world
...
2. [Must Quote Agent's Original Words and Actions]
   - Agent statements and behaviors are predictions of future population behaviors
...
3. [Language Consistency - Quoted Content Must Be Translated to Report Language]
   ...
   - When quoting English or mixed Chinese-English content from tool returns, you must translate it to fluent Chinese before writing to the report
   - Maintain original meaning during translation, ensure natural and smooth expressions
   - This rule applies to both body text and quote blocks (> format) content

4. [Faithfully Present Prediction Results]
   - Report content must reflect simulation results in the simulation world representing the future
...
- Do not use any Markdown headings in sections (#, ##, ###, ####, etc.)
...
- Use **bold**, paragraph separation, quotes, and lists to organize content, but not headings
...
Weibo, as the first scene of public opinion, undertakes the core function of initial information release:
...
[Tool Usage Suggestions - Please mix different tools, don't use just one]
- insight_forge: Deep insight analysis, automatically decompose questions and retrieve facts and relationships in multiple dimensions
...
- interview_agents: Interview simulation agents, get first-person perspectives and real responses from different roles

═══════════════════════════════════════════════════════════════
[Workflow]
═══════════════════════════════════════════════════════════════

Each time you reply, you can only do one of the following (cannot do both simultaneously):
...
The system will execute the tool and return the result to you. You do not need and should not write tool return results yourself.
...
3. Use Markdown format (but prohibit headings):
...
4. [Quote Format Rules - Must Be Separate Paragraphs]
   Quotes must be separate paragraphs with empty lines before and after, cannot be mixed in paragraphs:
   ...
   This reflects widespread public dissatisfaction.

   Wrong Format:
   The school's response was considered lacking in substance.> "The school's response pattern..." This reflects...
5. Maintain logical coherence with other sections
6. [Avoid Repetition] Carefully read the completed sections below, do not repeat describing the same information
7. [Emphasize Again] Do not add any titles! Use **bold** instead of subsection titles"""
...
- If information is sufficient: Output section content starting with "Final Answer:" (must quote above original text)
- If more information is needed: Call a tool to continue retrieval
═══════════════════════════════════════════════════════════════"""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Note] You only called {tool_calls_count} times, at least {min_tool_calls} required."
    "Please call tools again to get more simulation data, then output Final Answer. {unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently only called {tool_calls_count} times, at least {min_tool_calls} required."
...
[Rules]
1. Prioritize answering questions based on above report content
2. Answer questions directly, avoid lengthy thought expositions
...
class ReportAgent:
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "Interview topic or requirement description (e.g., 'Understand student views on dormitory formaldehyde incident')",
    ...
    # Valid tool name set, used for validation when parsing bare JSON fallback
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only try when format 1 doesn't match, to avoid mis-matching JSON in body text
        ...
        # Response may contain thought text + bare JSON, try to extract last JSON object
    ...
    def _generate_section_react(
        ...
        4. Repeat until information is sufficient or max times reached
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returns None, break loop and force finish
                break
            ...
            if has_final_answer:
                ...
                if tool_calls_count < min_tool_calls:
                    ...
                    unused_hint = f"(These tools haven't been used yet, recommend using them: {', '.join(unused_tools)})" if unused_tools else ""
                    messages.append({
                        "role": "user",
            ...
            if has_tool_calls:
                # Tool quota exhausted → clearly inform, require output Final Answer
                ...
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM tries to call {len(tool_calls)} tools, only executing first: {call['name']}")
    ...
    def generate_report(
        ...
        """
        Generate complete report (real-time section output)

        Each section is saved immediately after generation, no need to wait for entire report completion.
        File structure:
        reports/{report_id}/
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Start from which line (for incremental fetch, 0 means from beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if it's a duplicate title with section title (skip duplicates within first 5 lines)
                if i < 5:
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assemble complete report from saved section files and clean up titles
        """
        folder = cls._get_report_folder(report_id)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if it's duplicate title (same content appears within 5 consecutive lines)
                is_duplicate = False
                ...
                if is_duplicate:
                    # Skip duplicate title and empty lines after it
                    i += 1
        ...
        # Clean up consecutive multiple empty lines (keep max 2)
        result_lines = []
        empty_count = 0
