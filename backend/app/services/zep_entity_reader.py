...
Reads nodes from Zep graph and filters nodes matching predefined entity types
"""
...
class ZepEntityReader:
    ...
    2. Filter nodes matching predefined entity types (nodes with labels other than just Entity)
    ...
    def _call_with_retry(
        ...
        Args:
            func: Function to execute (lambda or callable with no parameters)
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all nodes from the graph (paginated fetch)
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all edges from the graph (paginated fetch)
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If a node's Labels only contains "Entity", skip it as it doesn't match our predefined types
        - If a node's Labels contains tags other than "Entity" and "Node", keep it as it matches predefined types
        ...
        for node in all_nodes:
            ...
            # Filter logic: Labels must contain tags other than "Entity" and "Node"
        ...
        logger.info(f"Filtering completed: total nodes {total_count}, matching {len(filtered_entities)}, "
    ...
    def get_entity_with_context(
        ...
        """
        Get a single entity with its complete context (edges and related nodes, with retry mechanism)
