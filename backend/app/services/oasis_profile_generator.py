...
Optimizations:
1. Call Zep retrieval function to enrich node info
2. Optimize prompts to generate very detailed profiles
...
class OasisProfileGenerator:
    ...
    Convert entities in Zep graph to Agent Profile format required by OASIS simulation

    Optimizations:
    1. Call Zep retrieval function to enrich node info
    2. Generate very detailed profiles (including basic info, career, personality traits, social media behavior, etc.)
    3. Distinguish individual entities from abstract group entities
    """
    ...
    # Individual entity types (need to generate specific persona)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group/institution entity types (need to generate representative persona)
    GROUP_ENTITY_TYPES = [
    ...
    def __init__(
        ...
        # Zep client for retrieval to get rich context
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep doesn't have a built-in hybrid search interface, need to search edges and nodes separately then merge results.
        Use parallel requests to search simultaneously for efficiency.
        ...
        try:
            ...
            logger.info(f"Zep hybrid search completed: {entity_name}, got {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 3. Add related nodes detailed information
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Check if it's an individual entity type"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES

    def _is_group_entity(self, entity_type: str) -> bool:
        """Check if it's a group/institution entity type"""
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if finish_reason == 'length':
                    logger.warning(f"LLM output truncated (attempt {attempt+1}), trying to fix...")
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if last char is not quote/comma/close bracket, string might be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            def fix_string_newlines(match):
                s = match.group(0)
                # Replace actual newlines in strings with space
                s = s.replace('\n', ' ').replace('\r', ' ')
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, maximally restoring existing reality. Must return valid JSON format, all string values cannot contain unescaped newlines."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate detailed social media user persona for entity, maximally restoring existing reality.
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   - Basic info (age, occupation, education background, location)
   - Character background (important experiences, event connection, social relationships)
   ...
   - Stance and views (attitude towards topic, content that may anger/move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of persona, introduce this individual's connection to the event, and their existing actions/reactions in the event)
...
Important:
- All field values must be strings or numbers, don't use newlines
- persona must be a coherent text description
- Use Chinese (except gender field must use English male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate detailed social media account setting for institution/group entity, maximally restoring existing reality.
...
2. persona: Detailed account setting description (2000 characters of plain text), must include:
   ...
   - Content posting characteristics (content types, posting frequency, active time periods)
   ...
   - Institution memory (important part of persona, introduce this institution's connection to the event, and its existing actions/reactions in the event)
3. age: Always fill 30 (virtual age for institutional accounts)
4. gender: Always fill "other" (institutional accounts use "other" for non-individual)
5. mbti: MBTI type to describe account style, e.g., ISTJ represents rigorous/conservative
6. country: Country (e.g., "China")
...
- persona must be a coherent text description, don't use newlines
- Use Chinese (except gender field must use English "other")
...
- Institutional account speech must match its identity positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "mbti": "ISTJ",  # Institution style: rigorous/conservative
                "country": "China",
                "profession": "Media",
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: Real-time write file path (if provided, write each as generated)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Real-time write file (even for fallback personas)
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Real-time output generated persona to console (full content, not truncated)"""
        separator = "-" * 70
        ...
        # Only output to console (avoid duplication, logger won't output full content)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Save Profile to file (choosing correct format based on platform)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile in CSV format (compliant with OASIS official requirements)
