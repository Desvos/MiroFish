...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        "dead": 0.05,      # Almost no activity at dawn
        "morning": 0.4,    # Gradually active in morning
        "work": 0.7,       # Work hours - medium
        "peak": 1.5,       # Evening peak
        "night": 0.5       # Late night decrease
    }
}
...
class TimeSimulationConfig:
    """Time simulation config (based on Chinese daily schedule)"""
    ...
    # Peak hours (evening 19-22, most active time for Chinese)
...
class PlatformConfig:
    ...
    # Viral threshold (how many interactions before triggering spread)
...
class SimulationConfigGenerator:
    ...
    Use LLM to analyze simulation requirements, document content, graph entity info,
    automatically generate optimal simulation parameter config

    Use step-by-step generation strategy:
    ...
    def generate_config(
        ...
        """
        logger.info(f"Start intelligent generation of simulation config: simulation_id={simulation_id}, entity_count={len(entities)}")
        ...
        logger.info("Assigning suitable Agents to initial posts...")
    ...
    def _call_llm_with_retry(self, prompt: str, system_prompt: str) -> Dict[str, Any]:
        """LLM call with retry, includes JSON fixing logic"""
        import re
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    temperature=0.7 - (attempt * 0.1)  # Decrease temperature on each retry
...
### Basic Principles (for reference only, adjust based on specific events and participants):
- Target users are Chinese, need to align with Beijing time schedule
...
- Evening 19-22: peak time (activity coefficient 1.5)
- After 23: activity decreases (activity coefficient 0.5)
- General pattern: low at dawn, increasing in morning, medium during work, peak in evening
- **Important**: The following example values are for reference only, you need to adjust based on event nature and participant characteristics
  - Example: Student groups may peak at 21-23; media active all day; official agencies only during work hours
  - Example: Breaking news may cause discussions late at night, off_peak_hours can be shortened
...
- reasoning (string): Brief explanation of configuration"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format, time config must match Chinese daily schedule."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        ...
        return {
            "total_simulation_hours": 72,
            "minutes_per_round": 60,  # 1 hour per round, speed up time flow
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        # Validate and fix: ensure doesn't exceed total agent count
...
Example: Official statements should be posted by Official/University types, news by MediaOutlet, student opinions by Student.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        Match most suitable agent_id based on each post's poster_type
        """
        ...
        # Track used agent index for each type to avoid reusing same agent
...
- **Time should match Chinese daily schedule**: almost no activity 0-5 AM, most active 19-22
- **Official agencies** (University/GovernmentAgency): low activity (0.1-0.3), active during work hours (9-17), slow response (60-240 min), high influence (2.5-3.0)
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": <active hours list, consider Chinese schedule>,
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON, config must match Chinese daily schedule."

        try:
            result = self._call_llm_with_retry(prompt, system_prompt)
            llm_configs = {cfg["agent_id"]: cfg for cfg in result.get("agent_configs", [])}
        except Exception as e:
            logger.warning(f"Agent config batch LLM generation failed: {e}, using rule generation")
            llm_configs = {}
        ...
        for i, entity in enumerate(entities):
            ...
            # If LLM didn't generate, use rules
            if not cfg:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent config based on rules (Chinese daily schedule)"""
