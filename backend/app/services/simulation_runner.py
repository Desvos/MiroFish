...
class SimulationRunState:
    ...
    # Independent rounds and simulated time for each platform (for parallel dual-platform display)
    ...
    # Platform completion status (detected by checking for simulation_end events in actions.jsonl)
...
class SimulationRunner:
    ...
    2. Parse execution logs and record each Agent's actions
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum simulation rounds (optional, used to truncate long simulations)
        ...
        # Determine which script to run (scripts are located in backend/scripts/ directory)
        if platform == "twitter":
        ...
        try:
            ...
            # Create main log file to avoid stdout/stderr pipe buffer full causing process blocking
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')

            # Set subprocess environment variables to ensure UTF-8 encoding on Windows
            # This fixes issues with third-party libraries (like OASIS) not specifying encoding when reading files
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ support, makes all open() use UTF-8 by default
            ...
            # Use start_new_session=True to create new process group, ensuring os.killpg can terminate all child processes
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group to ensure all related processes can be terminated when server closes
            )
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform was run, check only that platform
                                    # If two platforms were run, both need to complete
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines if a platform is enabled by checking if the corresponding actions.jsonl file exists
        ...
        # Check which platforms are enabled (determined by whether file exists)
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer record's platform, otherwise use default platform
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (used to force restart simulation)
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is anything to clean up (avoid empty process printing useless logs)
        has_processes = bool(cls._processes)
        has_updaters = bool(cls._graph_memory_enabled)

        if not has_processes and not has_updaters:
            return  # Nothing to clean up, silently return
    ...
    def register_cleanup(cls):
        ...
        Called when Flask application starts to ensure all simulation processes are cleaned up when server shuts down
        """
        ...
        # In Flask debug mode, only register cleanup in reloader subprocess (the one actually running the app)
        # WERKZEUG_RUN_MAIN=true indicates it's the reloader subprocess
        # If not in debug mode, this environment variable doesn't exist, still need to register
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered to prevent subprocess from trying again
            return
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, will be overwritten by each interview item's platform)
                - "twitter": By default only interview Twitter platform
                - "reddit": By default only interview Reddit platform
    ...
    def close_simulation_env(
        ...
        Send close environment command to simulation, making it exit the command waiting mode gracefully
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (waiting for response timed out, environment may be closing)"
            }
    ...
    def get_interview_history(
        ...
        # If multiple platforms were queried, limit total
