...
Run Twitter and Reddit simulation simultaneously, read same config file
...
# This is to fix OASIS third-party library reading files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # This affects all open() calls without specifying encoding
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')

    # Reconfigure stdout to UTF-8 (fix console Chinese character garbling)
    ...
    # So we also need to monkey-patch built-in open function
    import builtins
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        # Only set default encoding for text mode (non-binary) when encoding not specified
...
# Add filter immediately when module is loaded, to ensure it takes effect before camel code runs
...
class ParallelIPCHandler:
    ...
    Manages both platform environments, handles Interview commands
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/not specified: Interview both platforms, return integrated results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Need to interview both platforms
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying real entity names in actions.jsonl instead of IDs like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from database and enrich with complete context information

    Args:
        db_path: Database file path
        last_rowid: Last read maximum rowid value (use rowid instead of created_at because different platforms have different created_at formats)
        agent_names: agent_id -> agent_name mapping

    Returns:
        (actions_list, new_last_rowid)
        - actions_list: List of actions, each containing agent_id, agent_name, action_type, action_args (with context information)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keep only key fields (keep full content, don't truncate)
            simplified_args = {}
            ...
            # Enrich context information (post content, username, etc.)
...
def _enrich_action_context(
    ...
    """
    Enrich action with context information (post content, username, etc.)
    ...
    """
    try:
        # Like/dislike post: enrich post content and author
        ...
        # Repost: enrich original post content and author
        ...
        # Quote post: enrich original post content, author and quote comment
        ...
        # Follow user: enrich followed user's name
        ...
        # Mute user: enrich muted user's name
        ...
        # Like/dislike comment: enrich comment content and author
        ...
        # Create comment: enrich post information being commented on
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM configuration for speedup during parallel simulation:
...
async def run_twitter_simulation(
    ...
    # Get Agent real name mapping from config file (use entity_name instead of default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # If agent not in config, use OASIS default name
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed rowid in database (use rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        # Get and log actual executed actions from database
    ...
    # Note: Don't close environment, keep for Interview use
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM config (if available, otherwise fall back to general config)
...
async def main():
    ...
    # Create shutdown event at start of main function to ensure entire program responds to exit signal
    ...
    if args.twitter_only:
        ...
        # Run in parallel (each platform uses independent log recorder)
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure proper exit when SIGTERM/SIGINT received

    Persistent simulation scenario: After simulation completes, don't exit, wait for interview commands
    When termination signal is received, need to:
    1. Notify asyncio loop to exit wait
    2. Give program chance to properly clean up resources (close database, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (give loop chance to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()

        # Don't call sys.exit() directly, let asyncio loop exit normally and clean up resources
        # Only force exit if signal received repeatedly
        else:
            print("强制退出...")
            sys.exit(1)
