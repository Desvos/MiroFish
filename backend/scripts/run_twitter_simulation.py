...
class UnicodeFormatter(logging.Formatter):
    """Custom formatter, convert Unicode escape sequences to readable characters"""
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally don't set max_tokens, let model decide)"""
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Use project root .env file config (highest priority):
        - LLM_API_KEY: API key
        - LLM_BASE_URL: API base URL
        - LLM_MODEL_NAME: Model name
        """
        # Prefer to read config from .env
...
async def main():
    ...
    # Initialize logging config (use fixed filename, clean old logs)
...
def setup_signal_handlers():
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Force exit only if signal received again
            print("强制退出...")
            sys.exit(1)
